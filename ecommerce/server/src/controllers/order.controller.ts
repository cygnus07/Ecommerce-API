...
export const orderController = {
  // Create new order
  createOrder: async (req: Request<{}, {}, CreateOrderRequest>, res: Response): Promise<void> => {
    try {
      ...
      for (const cartItem of cart.items) {
        ...
        let variant = null;
        if (cartItem.variant) {
          variant = product.variants.find(v => v._id.toString() === cartItem.variant.toString());
          ...
          if (variant.inventory < cartItem.quantity) {
            sendError(res, `Not enough stock for ${product.name} (${variant.options.map(o => o.value).join(', ')})`, 400, ErrorCodes.BAD_REQUEST);
            return;
          }
        } else {
          // Use the default variant if no variant specified
          variant = product.variants.find(v => v.isDefault);
  ...
  confirmOrder: async (req: Request<{}, {}, ConfirmOrderRequest>, res: Response): Promise<void> => {
    try {
  ...
  // Get user's orders
  getUserOrders: async (req: Request<{}, {}, {}, OrdersQueryParams>, res: Response): Promise<void> => {
    try {
      const userId = req.user._id;
  ...
  // Get order by ID
  getOrderById: async (req: Request<OrderIdParams>, res: Response): Promise<void> => {
    try {
  ...
  getAllOrders: async (req: Request<{}, {}, {}, OrdersQueryParams>, res: Response): Promise<void> => {
    try {
      const { page = 1, limit = 10, status } = req.query;
      const skip = (page - 1) * limit;
  ...
  updateOrderStatus: async (req: Request<OrderIdParams, {}, OrderStatusRequest>, res: Response): Promise<void> => {
    try {
      const { id: orderId } = req.params;
      ...
      switch (status) {
        case OrderStatus.SHIPPED:
          updateData['shipping.trackingNumber'] = req.body.trackingNumber;
          updateData['shipping.trackingUrl'] = req.body.trackingUrl;
          updateData['shipping.estimatedDelivery'] = req.body.estimatedDelivery;
          break;
  ...
  generateInvoice: async (req: Request<OrderIdParams>, res: Response): Promise<void> => {
    try {
      const { id: orderId } = req.params;
